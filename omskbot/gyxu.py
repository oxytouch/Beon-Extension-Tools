...
def postmsg (threadid, text, user=None):
  ...
  if user == None:
    curl.setopt(pycurl.URL, 'http://gyxu.com/p/add_comment.cgi')
  else:
    curl.setopt(pycurl.URL, 'http://'+user+'.gyxu.com/p/add_comment.cgi')
...
def gettopics(f,b, forum='anonymous', user=None):
  ...
  if user == None:
    for i in range(f,b):
      curl.setopt(pycurl.URL, 'http://gyxu.com/'+forum+'/'+str(i)+'.html')
      curl.perform()
  else:
    for i in range(f,b):
      curl.setopt(pycurl.URL, 'http://'+user+'.gyxu.com/'+str(i)+'.html')
      curl.perform()
  curl.close()
  return string.getvalue()
  string.close()
