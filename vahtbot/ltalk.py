...
def postmsg (threadid, text, user=None):
  ...
  if user == None:
    curl.setopt(pycurl.URL, 'http://ltalk.ru/p/add_comment.cgi')
  else:
    curl.setopt(pycurl.URL, 'http://'+user+'.ltalk.ru/p/add_comment.cgi')
...
def gettopics(f,b, forum='anonymous', user=None):
  ...
  if user == None:
    for i in range(f,b):
      curl.setopt(pycurl.URL, 'http://ltalk.ru/'+forum+'/'+str(i)+'.html')
      curl.perform()
  else:
    for i in range(f,b):
      curl.setopt(pycurl.URL, 'http://'+user+'.ltalk.ru/'+str(i)+'.html')
      curl.perform()
  curl.close()
  return string.getvalue()
  string.close()

targetregexp = r"\/anonymous\/(\d*)-(\d*)\-[\w|\-]*([vb]-?i*-?r-?t|se(?:x|ks|kas)|eb(?:at|i)|t-?r-?a-?h|(?:-ja|ischu|hochu)-(?:m\-|j\-|zh\-|devushk|par(?:en|nja)|hozja)|ots[o|\-]s|rolevit|-sis[\-|e][kc]|v(?:-pop|du(?:i\-|va))|rabyn|otdams|orgi(?:ja|u)|droch|[ob]?liz(?:at\-|va[it])|hentai|shlju(?:hu|shk)|kisk[au]-(?:vsja|mokr)|do-orgazm|shali|min-?et|nakaz(?:iva|hi|at)|(?:parni|devushki)-kto-hochet|hoch(?:u|esh)-tak-)[\w|\-]*\-read\.shtml"
